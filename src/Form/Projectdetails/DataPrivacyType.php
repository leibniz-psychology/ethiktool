...
class DataPrivacyType extends TypeAbstract
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void {
        ...
        if ($dummyParams['isOnline'] ?? false) { // only if location is online
            // data online
    ...
    public function mapDataToForms(mixed $viewData, Traversable $forms): void {
        ...
        if (array_key_exists(self::processingNode,$forms)) {
            $forms[self::processingNode]->setData($viewData[self::processingNode]);
        }
        if (array_key_exists(self::createNode,$forms)) {
            ...
            if ($isTool) {
                ...
                if ($tempVal) {
                    ...
                    if (in_array($responsibility, [self::responsibilityOnlyOwn, self::privacyNotApplicable]) && in_array($transferOutside, [self::transferOutsideNo, self::privacyNotApplicable])) {
                        // data online (processing)
                        $this->setChosenArray($forms, $viewData, self::dataOnlineNode, [self::descriptionNode => self::dataOnlineProcessingNode]);
                        // data personal
                        ...
                        foreach (array_merge([''], array_key_exists(self::markingNode.self::markingSuffix, $viewData) ? [self::markingSuffix] : []) as $suffix) {
                            $marking = self::markingNode.$suffix;
                            $this->setChosenArray($forms, $viewData, $marking, [self::descriptionNode => $marking.self::descriptionCap, self::markingInternal => self::markingInternal.$suffix]);
