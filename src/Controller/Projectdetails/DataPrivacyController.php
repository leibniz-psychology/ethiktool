...
class DataPrivacyController extends ControllerAbstract
{
    ...
    public function showDataPrivacy(Request $request): Response
    {
        ...
        if ($hasCreate) {
            ...
            $isOnline = $pageArray[self::locationNode][self::chosen]===self::locationOnline;
            ...
            if ($types!=='' && !array_key_exists(self::compensationNo, $types)) {
                ...
                foreach (array_diff(self::compensationTypes, [self::compensationNo, self::compensationOther]) as $type) {
                    if (array_key_exists($type, $types)) {
                        $awarding = $compensationArray[$type.self::awardingNode];
        ...
        $dataPrivacy = $this->createFormAndHandleRequest(DataPrivacyType::class,$this->xmlToArray($privacyNode),$request, $hasCreate ? [self::dummyParams => ['isCompensationCode' => $this->checkCompensationAwarding($compensationArray), 'isOnline' => $isOnline]] : []);
        if ($dataPrivacy->isSubmitted()) {
            ...
            return $this->saveDocumentAndRedirect($request,!$hasCreate || $isNotLeave ? $appNode : $appNodeNew, $hasCreate && $isNotLeave ? $appNodeNew : null); // appNodeNew is only defined if $hasCreate is true
        }
        return $this->render('Projectdetails/dataPrivacy.html.twig',
            $this->setRenderParameters($request,$dataPrivacy, $hasCreate ?
